...
#    text_ = text_ + """#SBATCH --comment="#SBATCH --array-0,1,3" """
    ...
    text_ = text_ + """GNAMEIN="${INFOLDER}${GNAME}"
        ...
        cp ${GNAMEIN}-in.gro ${GNAME}-in.gro
        cp ${GNAMEIN}.top ${GNAME}.top
        cp ${GNAMEIN}.ndx ${GNAME}.ndx
        #avoid overwriting file
        if [ ! -f ${GNAME}.mdp ];
        then
            cp ${GNAMEIN}%s.mdp ${GNAME}.mdp
        fi
        """ % (suffix)
    text_ = text_ + """grompp_d -c ${GNAME}-in.gro -p ${GNAME}.top -n ${GNAME}.ndx -f ${GNAME}.mdp -o ${GNAME}.tpr -maxwarn 15
...
def generate(base_name, submit):
    dir = '/scratch/jtp4kc/simulations/t41methrand'
#    dir = os.getcwd()
    for i in range(20):
        suffix = '_{0:0>2}'.format(i)
        mdp_out = open(os.path.join(dir, "%s-in" % base_name, "%s%s.mdp" % (base_name, suffix)), 'w')
        for line in open(os.path.join(dir, "%s-in" % base_name, "%s.mdp" % base_name)):
            if 'seed' in line:
                halves = line.split("=")
